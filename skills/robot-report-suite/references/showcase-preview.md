...
2. 导出每页 PNG，再生成不裁切、不改变页面顺序的联系表；封面、基础信息页、复杂表格和最后一页必须单独目视检查。
3. 展示重点页时，使用原始页图，不把多页内容拼接为看似单页的图；任何裁切都要标注“局部预览”。
4. 用户反馈版式问题时，回到版本化 `.docx` 修订并重新导出 PDF；禁止在 PNG 上修图后把它当作报告修订。
