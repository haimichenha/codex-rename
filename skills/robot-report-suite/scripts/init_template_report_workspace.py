...
It copies a template into both the protected input area and a versioned output
copy, then creates the inventory/format-contract scaffolding.  It never edits
the user template and refuses to overwrite an existing deliverable.
"""
...
def main() -> int:
    ...
    parser.add_argument("--output-name", help="Versioned output filename; defaults to <template-stem>_v1.docx")
    ...
    output_name = args.output_name or f"{template.stem}_v1.docx"
    if Path(output_name).suffix.lower() != ".docx":
        raise SystemExit("--output-name must end in .docx")
    for directory in ("input", "evidence", "figures/source", "figures/export", "sections", "output", "qa"):
        (workspace / directory).mkdir(parents=True, exist_ok=True)
    input_copy = workspace / "input" / template.name
    ...
    manifest = {
        ...
        "next_gate": "Complete format-contract.md before inserting new report content.",
    }
