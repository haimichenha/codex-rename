...
description: 创建、重构、审稿和渲染技术答辩、项目汇报与毕业设计 PowerPoint。用户提供素材、现有 .pptx、演讲主题、视觉参考，或要求提升 PPT 的审美、版式、信息组织、讲述备注和投影可读性时使用；可选地导入已定稿报告的证据包，也支持只基于原始材料独立工作。不要用于撰写 Word 或实验报告正文。
---
...
按 `references/production-system.md` 逐页制作，按 `references/render-delivery.md` 导出单页和 contact sheet，再按 `references/review-rubric.md` 审稿。先纠正结论、边界和证据，再处理阅读路径、裁切和材质，最后才加 Finish。
